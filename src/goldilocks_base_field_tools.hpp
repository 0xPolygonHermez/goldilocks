...
inline void Goldilocks::fromString(Element &result, const std::string &in1, int radix)
{
    ...
    aux = (aux + GOLDILOCKS_PRIME) % GOLDILOCKS_PRIME;
...
{
    mpz_class aux = (scalar + GOLDILOCKS_PRIME) % GOLDILOCKS_PRIME;
    result.fe = aux.get_ui();
};
...
inline void Goldilocks::toS64(int64_t &result, const Element &in1)
{
    ...
    mpz_class out = (unsigned long int) Goldilocks::toU64(in1);

    mpz_class maxInt((GOLDILOCKS_PRIME - 1) / 2);

    if (out > maxInt)
    {
        mpz_class onegative = GOLDILOCKS_PRIME - out;
...
{
    mpz_class out = mpz_class((unsigned long int) Goldilocks::toU64(in1));
