...
void  NTT_Goldilocks::ntt_block_iters(Goldilocks::Element *dst, Goldilocks::Element *src, u_int64_t size, u_int64_t offset_cols, u_int64_t ncols, u_int64_t ncols_all,u_int64_t nphase,Goldilocks::Element *aux)
{
    ...
    reversePermutation_block(a2, src, size, offset_cols, ncols, ncols_all);
        ...
        for (u_int64_t b = 0; b < nBatches; b++)
        {
            ...
            for (u_int64_t kk = 0; kk < niters; ++kk)
            {
                for (u_int64_t x = 0; x < srcWidth; x++)
                {
                    ...
                    std::memcpy(&a2[offset_dstY], &a[offset_src], ncols * sizeof(u_int64_t));
                }
            }
        }
        tmp = a2;
...
void NTT_Goldilocks::NTT_Block(Goldilocks::Element *src, u_int64_t size, u_int64_t ncols, u_int64_t nphase, u_int64_t nblock)
{
    ...
    for(int ib=0; ib<nblock; ++ib){
        ...
        if(ib < ncols_res) aux_ncols += 1;
        Goldilocks::ntt_block_iters(dst,src,size,offset_cols,aux_ncols,ncols,nphase,aux);
        if(nblock > 1){
            #pragma omp parallel for static
            for(int ie=0; ie<size; ++ie){
                offset2 = ie*ncols+offset_cols;
...
void NTT_Goldilocks::reversePermutation_block(Goldilocks::Element *dst, Goldilocks::Element *src, u_int64_t size, u_int64_t offset_cols, u_int64_t ncols, u_int64_t ncols_all)
{
    uint32_t domainSize = log2(size);
#pragma omp parallel for schedule(static)
    for (u_int64_t i = 0; i < size; i++)
    {
        ...
        u_int64_t offset_src = i * ncols_all + offset_cols;
        u_int64_t offset_dst = r * ncols;
        std::memcpy(&dst[offset_dst], &src[offset_src], ncols * sizeof(u_int64_t));
    }
}
